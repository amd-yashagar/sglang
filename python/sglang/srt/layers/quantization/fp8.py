...
class Fp8LinearMethod(LinearMethodBase):
    ...
    def _process_mxfp8_linear_weight_scale(self, layer: Module) -> None:
        ...
        if backend.is_flashinfer_trtllm():
            ...
            configs = {
                m: get_flydsl_mxfp8_config(m, n, k) for m in MXFP8_FLYDSL_M_VALUES
            }
    ...
    def _get_mxfp8_flydsl_runtime_buffers(
        ...
        *,
        ...
        return {
            ...
            "activation_scale_shuffled": pooled["activation_scale_shuffled"][:padded_m],
            "output_buffer": pooled["output_buffer"][:m],
            "splitk_workspace": (
                workspace[:workspace_elems].view(split_k, m, n) if split_k > 1 else None
            ),
        }
    ...
    def apply(
        ...
        if self.use_mxfp8:
            backend = self.mxfp8_dense_backend
            if backend is not None and backend.is_aiter():
                from sglang.kernels.ops.quantization.mxfp8_amd_gfx95 import (
                    dot_scaled_mxfp8_blockscaled_linear,
                    get_flydsl_mxfp8_config,
                )

                input_value, input_scale = x if isinstance(x, tuple) else (x, None)
                ...
                if config is not None and m <= 64:
                    ...
                    try:
                        ...
                        if (
                            ...
                            or (
                                decode_graph.bs is not None and m not in decode_graph.bs
                            )
                            or (
                ...
                if signature not in _LOGGED_MXFP8_FLYDSL_SIGNATURES:
                    log_info_on_rank0(
                        ...
                        f"M={m} N={n} K={k} kernel={config.get('kernelName')}",
                    )
...
class Fp8MoEMethod(FusedMoEMethodBase):
    ...
    def __init__(self, quant_config: Fp8Config):
        ...
        self.convert_mxfp8_to_block = (
            self.use_mxfp8 and _mxfp8_to_block_fp8_required and not native_aiter_mxfp8
        )
        self.weight_block_size = self.quant_config.weight_block_size
    ...
    def _process_mxfp8_moe_weights(self, layer: Module, quantize: bool = True) -> None:
        ...
        if _use_aiter and runner_is_aiter:
            gate_up_interleaved = (
                self.use_mxfp8 or envs.SGLANG_USE_AITER_MOE_GU_ITLV.get()
            )
