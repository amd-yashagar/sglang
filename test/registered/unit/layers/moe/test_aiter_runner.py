...
def test_standard_ep_rejects_unpaired_aiter(monkeypatch):
    ...
    with pytest.raises(NotImplementedError, match="has_fake_topk_slot"):
        runner.run(
            _runner_input(),
...
def test_aiter_runner_maps_gated_silu_to_clamped_swiglu(monkeypatch):
    ...
    runner.run(
        _runner_input(),
        _quant_info(w13_weight=torch.empty((2, 8, 2), dtype=torch.float8_e4m3fn)),
        running_state={},
    )
