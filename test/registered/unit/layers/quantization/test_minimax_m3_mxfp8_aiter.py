...
def test_dense_mxfp8_rejects_missing_aiter_flydsl(monkeypatch):
    import builtins
    ...
    def reject_aiter_flydsl(name, *args, **kwargs):
        if name == "aiter.ops.flydsl":
            raise ImportError("missing FlyDSL GEMM")
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", reject_aiter_flydsl)
    with pytest.raises(RuntimeError, match="MX-scale preshuffle GEMM is unavailable"):
...
def test_mxfp8_aiter_backend_dispatches_to_flydsl(monkeypatch):
    ...
    assert fp8_utils.resolve_mxfp8_dense_gemm_backend().is_aiter()
    assert fp8_utils.dispatch_w8a8_mxfp8_linear() is flydsl_mxfp8_blockscaled_linear
...
def test_mxfp8_flydsl_unknown_topology_keeps_canonical_weights(monkeypatch):
    ...
    fake_shuffle.shuffle_scale_a16w4 = lambda *args, **kwargs: args[0]
    ...
    monkeypatch.setattr(mxfp8_amd_gfx95, "get_flydsl_mxfp8_config", lambda *args: None)
    ...
    method.mxfp8_dense_backend = fp8_utils.Mxfp8DenseGemmBackend.AITER
